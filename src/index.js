// ============================================
// MULTI-TIME LINK OPENER - v4.0.0 (FULL)
// ============================================
...
class IPManager {
  ...
  getEUIP(seed) { return this.getUKIP(seed); } // simplified
...
function getAllowedHosts(env) {
  ...
  if (raw === '*') return ['*'];
  // Default hosts (so the given link works even if env not set)
  ...
  // Combine defaults and env (env overrides if set)
  const combined = fromEnv.length ? fromEnv : defaults;
  return combined;
}
...
function errorResponse(error, status, extra = {}) {
  return json({ success: false, error }, status, extra);
}
...
async function handleValidate(request, env) {
  ...
  if (!body || typeof body.url !== 'string' || !body.url.trim()) return errorResponse('URL is required', 400);
...
async function handleTest(request, env) {
  ...
  if (!limit.allowed) return errorResponse('Too many requests', 429, { 'Retry-After': String(limit.retryAfterSeconds) });
...
async function handleRequest(request, env) {
  ...
  // Placeholder analytics endpoints (simplified)
  ...
  if (method === 'POST' && path === '/test-advanced') {
    // Redirect to /test for simplicity, or implement full advanced later
    return handleTest(request, env);
  }
  if (method === 'POST' && path === '/test-stealth') {
    // Use stealth level
    const req = new Request(request, { body: request.body });
    // We'll modify body to add stealth level (parse, set bypassLevel, then call handleTest)
    // For simplicity, we'll call handleTest after modifying body.
    // But since we can't easily modify body, we'll just call a new handler.
    // To keep it simple, we'll just call handleTest but set bypassLevel in body if possible.
    // Actually we can parse body, add bypassLevel, then call handleTest.
    try {
      ...
      // We need to override the body, but we can't easily modify request body.
      // So we'll handle it directly.
      return handleTest(newRequest, env);
    } catch {
      return handleTest(request, env);
    }
  }
